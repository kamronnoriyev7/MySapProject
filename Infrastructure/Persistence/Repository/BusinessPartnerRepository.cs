...
namespace MySapProject.Infrastructure.Persistence.Repository
{
    public class BusinessPartnerRepository : IBusinessPartnerRepository
    {
        ...
        public async Task<string> GetFilteredAsync(
             string? filter = null,
             string? select = null,
             string? orderBy = null,
             int? top = null,
        ...
        public async Task<string> GetByIdAsync(string cardCode)
        {
        ...
        public async Task<string> CreateAsync(BusinessPartner partner)
        {
        ...
        public async Task<string> UpdateAsync(string cardCode, BusinessPartner partner)
        {
            var json = JsonSerializer.Serialize(partner);
            var content = new StringContent(json, Encoding.UTF8, "application/json");

            var response = await _httpClient.PatchAsync($"BusinessPartners('{cardCode}')", content);
            await response.EnsureSuccessOrThrowAsync();
            return await response.Content.ReadAsStringAsync();
        }

        public async Task<string> DeleteAsync(string cardCode)
        {
